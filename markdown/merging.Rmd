...
fs <- list(
  ...
  inflation    = file.path("data", "inflation.RDS"))
```
...
inflation    <- readRDS(fs$inflation)
```
...
```{r add_inflation}
inflation$ym <- make_ym(inflation$caldt)

# merge
macro_dat <- merge(
  macro_dat, inflation[, c("ym", "cpiind")], by = "ym", all.x = TRUE)
stopifnot(!anyNA(macro_dat$cpiind))
```
...
Compute other deflated size measures
...
final[
  , `:=`(
    ...
    atq_defl  = atq  / cpiind * 100, 
    # deflated pledgeable assets as in David et al. (2013). It is current assets
    # plus Property, Plant and Equipment in the aforementioned paper. We only 
    # include current assets
    actq_defl = actq / cpiind * 100)]

# plot mean vs. time to check that level is somewhat stable
local({
  func <- function(trans, stats, xlab_suf){
    par(mar = c(5, 4, 1, 1), mfcol = c(1, 2))
    f <- function(s){
      z <- tapply(trans(final[[s]]), final$tstop, stats, na.rm = TRUE)
      z <- z / z[1] # scale to index 1 at start
      
      plot(z, type = "l", ylab = paste(s, xlab_suf))
    }
    for(s in c("atq_defl", "actq_defl")){
      f(s)
      f(gsub("_defl", "", s))
  ...
  func(identity, mean, "(mean)")
  func(identity, median, "(median)")
  func(function(x) suppressWarnings(log(x)), mean, "(log)")
})

# check correlations
cor(final[, .(rel_size, log(atq_defl), log(actq_defl + 1))], 
    use = "complete.obs")
# after winsorizing
local({
  tmp <- final[, .(rel_size, log(atq_defl), log(actq_defl + 1))]
